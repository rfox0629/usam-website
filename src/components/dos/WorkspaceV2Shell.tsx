...
export type WorkspaceV2Mode = "admin-preview" | "portal";
export type WorkspaceV2Section = "fruit" | "home" | "meetings" | "more" | "people";

export type WorkspaceV2Query = {
  person?: string;
  q?: string;
  section?: string;
  status?: string;
...
export function normalizeWorkspaceV2Section(value: string | undefined): WorkspaceV2Section {
...
function buildSectionHref(
  basePath: string,
  section: WorkspaceV2Section,
  params: Record<string, string | null | undefined> = {},
  hash = "",
  includePreviewParam = false,
) {
  const searchParams = new URLSearchParams();

  if (includePreviewParam) {
    searchParams.set("viewAs", "workspace_user");
  }
  ...
  return (
    <div className="rounded-2xl border border-[#BFDBFE] bg-[#EBF2FF] px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#64748B]" style={{ fontFamily: workspaceFont.ui }}>
          {label}
        </p>
    ...
    <section className="rounded-[28px] bg-white p-4 shadow-[0_18px_44px_rgba(15,23,42,0.08)] sm:p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-bold text-[#0F172A]" style={{ fontFamily: workspaceFont.display }}>
            Your circles
          </h2>
        </div>
        {mode === "admin-preview" ? (
...
function SectionTabs({
  ...
  activeSection: WorkspaceV2Section;
  basePath: string;
  includePreviewParam: boolean;
}) {
  const tabs: Array<{ icon: LucideIcon; label: string; section: WorkspaceV2Section }> = [
      ...
      {tabs.map((tab) => {
        ...
        return (
          <Link
            ...
            href={buildSectionHref(basePath, tab.section, {}, "", includePreviewParam)}
            key={tab.section}
          >
            <Icon className="h-4 w-4" aria-hidden="true" />
            {tab.label}
    ...
    <section className="scroll-mt-24 rounded-[26px] bg-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-5" id="person-detail">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <h2 className="mt-1 break-words text-2xl font-bold text-[#0F172A]" style={{ fontFamily: workspaceFont.display }}>
            {person.name}
          </h2>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
        <Link className="inline-flex min-h-11 items-center justify-center rounded-full bg-[#2563EB] px-4 text-sm font-semibold text-white" href={logMeetingHref}>
          Log Meeting
        </Link>
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
      <section className="rounded-[26px] bg-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Link className="inline-flex min-h-10 items-center justify-center rounded-full bg-[#2563EB] px-4 text-sm font-semibold text-white" href={addPersonHref}>
            Add Person
          </Link>
        </div>

        <form action={basePath} className="mt-5" method="get">
          {includePreviewParam ? <input name="viewAs" type="hidden" value="workspace_user" /> : null}
          ...
          {statusOptions.map((status) => {
            ...
            return (
              <Link
                ...
                href={buildSectionHref(basePath, "people", { q: searchQuery, status }, "", includePreviewParam)}
                key={status || "all"}
              >
        ...
        <div className="mt-5 space-y-2">
          {filteredPeople.length > 0 ? filteredPeople.map((person) => (
            <FieldPersonCard
              href={buildSectionHref(basePath, "people", { person: person.id, q: searchQuery, status: statusFilter }, "#person-detail", includePreviewParam)}
              key={person.id}
              person={person}
      ...
      <PersonQuickView
        dosHref={dosHref}
        logMeetingHref={logMeetingHref}
        meetings={selectedPersonMeetings}
        person={selectedPerson}
        prayerHref={prayerHref}
      />
    </div>
  );
}
  ...
  return (
    <section className="rounded-[28px] bg-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link className="inline-flex min-h-10 items-center justify-center rounded-full bg-[#2563EB] px-4 text-sm font-semibold text-white shadow-[0_12px_28px_rgba(37,99,235,0.18)] transition hover:bg-[#1D4ED8]" href={logMeetingHref}>
          Log Meeting
        </Link>
      </div>
  ...
  return (
    <section className="rounded-[28px] bg-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link className="inline-flex min-h-10 items-center justify-center rounded-full bg-[#2563EB] px-4 text-sm font-semibold text-white shadow-[0_12px_28px_rgba(37,99,235,0.18)] transition hover:bg-[#1D4ED8]" href={openFruitHref}>
          Open Fruit
        </Link>
      </div>
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
...
function MoreSection({
  ...
  const portalTools = [
      ...
      : []),
    {
      ...
      disabled: !preview.prayer.teamEnabled,
      href: "/prayer",
      icon: Heart,
      label: "Prayer Team",
    ...
    {
      description: "Upload a CSV to add people to your field.",
      disabled: true,
      href: "#",
      icon: Import,
    ...
    {
      description: "Workspace identity and enabled tools.",
      disabled: true,
      href: "#",
      icon: Settings,
      label: "Settings",
    ...
    <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="min-w-0 space-y-5">
        <section className="min-w-0 rounded-[26px] bg-white p-4 shadow-[0_16px_40px_rgba(15,23,42,0.08)] sm:p-5">
          ...
          <h2 className="mt-2 text-xl font-bold text-[#0F172A]" style={{ fontFamily: workspaceFont.display }}>
            Workspace status
          </h2>
          <div className="mt-4">
...
export function WorkspaceV2Shell({
  ...
  preview: WorkspacePreviewData;
  publicProfileHref: string;
  query: WorkspaceV2Query;
  workspaceHref: string;
}) {
  const isAdminPreview = mode === "admin-preview";
  const includePreviewParam = isAdminPreview;
  const activeSection = normalizeWorkspaceV2Section(query.section);
  ...
  const sectionHref = (
    ...
    params: Record<string, string | null | undefined> = {},
    hash = "",
  ) => buildSectionHref(basePath, section, params, hash, includePreviewParam);
  const addPersonHref = isAdminPreview ? `${workspaceHref}&tab=people` : dosHref;
  const logMeetingHref = isAdminPreview ? `${workspaceHref}&tab=meetings` : dosHref;
  const openFruitHref = isAdminPreview ? `${workspaceHref}&tab=fruit` : sectionHref("fruit");
  const prayerHref = isAdminPreview ? "/admin/prayer-team" : "/prayer";
  const quickActions = [
    { href: addPersonHref, icon: UserPlus, label: "Add Person", meta: "People" },
    { href: logMeetingHref, icon: Plus, label: "Log Meeting", meta: "Meetings" },
    { href: sectionHref("people"), icon: Search, label: "Search", meta: "Find someone" },
    { href: openFruitHref, icon: Flame, label: "Record Fruit", meta: "Outcomes" },
  ];
  const visibleMembers = preview.members.slice(0, 4);
  const activityHref = (item: WorkspacePreviewData["activity"][number]) => {
    if (isAdminPreview) {
      return item.href;
    }

    if (item.label === "Meeting") {
      return sectionHref("meetings");
    }

    if (item.label === "Fruit" || item.label === "Review") {
      return sectionHref("fruit");
    }

    if (item.label === "Prayer") {
      return sectionHref("more");
    }

    return sectionHref("people");
  };

  return (
    <div
      className={isAdminPreview
        ? "overflow-hidden rounded-[34px] border border-[#E2E8F0] bg-[#FAFBFD] text-[#0F172A] shadow-[0_24px_80px_rgba(15,23,42,0.18)]"
        : "min-h-screen bg-[#FAFBFD] text-[#0F172A]"}
    >
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#2563EB]" style={{ fontFamily: workspaceFont.ui }}>
                {isAdminPreview ? "Workspace v2" : "Workspace"}
              </p>
              {isAdminPreview ? (
            ...
            <h1 className="mt-2 max-w-3xl break-words text-[2.35rem] font-bold leading-none text-[#0F172A] sm:text-5xl" style={{ fontFamily: workspaceFont.display }}>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-24 pt-5 sm:px-6 lg:px-7 lg:pb-8 lg:pt-7">
        <SectionTabs activeSection={activeSection} basePath={basePath} includePreviewParam={includePreviewParam} />
        ...
        {activeSection === "home" ? (
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1.05fr)_360px]">
            ...
            <aside>
              <section>
                <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.22em] text-[#94A3B8]" style={{ fontFamily: workspaceFont.ui }}>
                  Quick Actions
                </p>
                  ...
                  preview.activity.map((item) => (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 block text-[10px] font-bold uppercase tracking-[0.14em] text-[#2563EB]" style={{ fontFamily: workspaceFont.ui }}>
                          {item.label}
                        </span>
                        <span className="mt-1 line-clamp-2 block text-sm leading-5 text-[#64748B]">{item.detail}</span>
                      </span>
            ...
            <section>
              ...
              <div className="space-y-2">
                <SecondaryTool href={sectionHref("people")} icon={Users} label="People" />
                <SecondaryTool href={sectionHref("meetings")} icon={CalendarDays} label="Meetings" />
                <SecondaryTool href={sectionHref("fruit")} icon={Flame} label="Fruit" />
                {preview.features.publicProfileEnabled ? (
                ...
                <SecondaryTool disabled={!isAdminPreview} href={isAdminPreview ? `${workspaceHref}&tab=people` : "#"} icon={Import} label="Imports" />
                <SecondaryTool disabled={!isAdminPreview} href={isAdminPreview ? `${workspaceHref}&tab=features` : "#"} icon={Settings} label="Settings" />
              </div>
            </section>
          </div>
          ...
          <PeopleSection
            ...
            includePreviewParam={includePreviewParam}
            logMeetingHref={logMeetingHref}
            preview={preview}
            prayerHref={prayerHref}
          ...
          <MeetingsSection logMeetingHref={logMeetingHref} preview={preview} />
        ) : activeSection === "fruit" ? (
          <FruitSection openFruitHref={openFruitHref} preview={preview} />
        ) : (
          <MoreSection
            mode={mode}
            preview={preview}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 gap-1 border-t border-[#E2E8F0] bg-white/90 px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden" aria-label={isAdminPreview ? "Workspace preview navigation" : "Workspace navigation"}>
        <BottomNavItem active={activeSection === "home"} href={sectionHref("home")} icon={Home} label="Home" />
        <BottomNavItem active={activeSection === "people"} href={sectionHref("people")} icon={Users} label="People" />
        <BottomNavItem active={activeSection === "meetings"} href={sectionHref("meetings")} icon={CalendarDays} label="Meetings" />
        <BottomNavItem active={activeSection === "fruit"} href={sectionHref("fruit")} icon={Flame} label="Fruit" />
        <BottomNavItem active={activeSection === "more"} href={sectionHref("more")} icon={BookOpen} label="More" />
      </nav>
    </div>
  );
}
