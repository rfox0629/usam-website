...
/**
 * Statuses a reader is allowed to see on the public site.
 *
 * Publication and sending are separate axes. The Operations review workflow
 * (draft -> ready_for_review -> test_sent -> approved -> scheduled -> sent)
 * describes readiness to mail; it does not make an issue public. `sent` is
 * readable because the "Read online" link inside a delivered email has to
 * resolve the moment it lands, and `published` covers an issue put on the site
 * without a mailing. Everything earlier is a draft and stays private, so an
 * unapproved issue can never be read by guessing its slug.
 */
...
export async function getPublishedNewsletters() {
  ...
  const { data, error } = await supabase
    ...
    .select(publicNewsletterColumns)
    .in("status", publiclyReadableNewsletterStatuses as unknown as string[])
...
export async function updateSubscriberPreferences(
  ...
  await recordSubscriberEvent(supabase, {
    ...
    payload: { selected_topics: Array.from(selectedTopics) },
    source: input.source,
