...
import { q3FieldUpdateAssetBaseUrl, q3FieldUpdateLinks, q3FieldUpdateSlug } from "./config";
import { renderQ3FieldUpdateEmail } from "./newsletter-q3-2026-template";
...
type RenderNewsletterEmailInput = {
  ...
  /**
   * Founder-review draft renders the notes that mark what still has to be
   * supplied. The donor send must render without them.
   */
...
export function renderNewsletterEmail({
  ...
  // Designed issues render themselves. They still take the archive, preference,
  // and unsubscribe URLs built above, so the footer contract is identical.
  if (newsletter.slug === q3FieldUpdateSlug) {
    const designed = renderQ3FieldUpdateEmail({
      archiveUrl,
