...
export function mapCommitmentRow(row: Record<string, unknown>, updates: Array<Record<string, unknown>> = []) {
  return {
    ...
    targetCount: typeof row.target_count === "number" ? row.target_count : null,
    targetDate: row.target_date ? String(row.target_date) : null,
    targetKind: typeof row.target_kind === "string" && isDosCommitmentTargetKind(row.target_kind) ? row.target_kind : null,
...
export function mapCommitmentUpdateRow(row: Record<string, unknown>) {
  return {
    ...
    subjectPersonId: row.subject_person_id ? String(row.subject_person_id) : null,
    subjectPersonName: row.subject_person_name ? String(row.subject_person_name) : null,
