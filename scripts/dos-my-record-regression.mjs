...
assert(workspaceRoute.includes("id: workspaceAccess.workspace.id") && workspaceRoute.includes("slug: workspaceAccess.workspace.slug"), "DOS route should load data from the authorized resolved workspace for Ryan, Dirk, and generic slugs.");
...
assert(canonicalTabsSource.includes("{ label: \"Calling\", value: \"calling\" }"), "Canonical My Record should expose Calling as a primary tab.");
assert(canonicalTabsSource.includes("{ label: \"Legacy\", value: \"legacy\" }"), "Canonical My Record should expose Legacy as a primary tab.");
