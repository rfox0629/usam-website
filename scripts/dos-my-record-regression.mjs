...
assert(loader.includes("dosMyRecordV2WorkspaceDisplayNames"), "Feature flag should support Ryan's Fox Family display-name identity.");
assert(loader.includes("dosMyRecordV2UserEmails"), "Feature flag should have an email fallback.");
assert(loader.includes("workspaceDisplayName?: string | null;"), "Feature flag should accept workspace display name for household/default workspace resolution.");
assert(loader.includes("workspacePublicSlug?: string | null;"), "Feature flag should accept public slug for canonical route/workspace resolution.");
assert(loader.includes("workspaceDisplayName: workspace.display_name"), "Loader should pass the resolved household display name to the V2 gate.");
assert(loader.includes("workspacePublicSlug: workspace.public_slug"), "Loader should pass the resolved household public slug to the V2 gate.");
assert(loader.includes("\"fox family\""), "Feature flag should include the Fox Family display label for Ryan's canonical workspace.");
assert(loader.includes("dosMyRecordV2WorkspaceSlugs.has(identifier) || dosMyRecordV2WorkspaceDisplayNames.has(identifier)"), "Feature flag should identify Ryan's workspace from slug, public slug, or display name.");
...
assert(route.includes("workspaceDisplayName: workspace.display_name") && route.includes("workspacePublicSlug: workspace.public_slug"), "My Record API saves should use the same Ryan/Fox Family V2 gate as the loader.");
assert(attachmentRoute.includes("workspaceDisplayName: workspace.display_name") && attachmentRoute.includes("workspacePublicSlug: workspace.public_slug"), "Assessment uploads should use the same Ryan/Fox Family V2 gate as the loader.");
assert(learningAttachmentRoute.includes("workspaceDisplayName: workspace.display_name") && learningAttachmentRoute.includes("workspacePublicSlug: workspace.public_slug"), "Learning uploads should use the same Ryan/Fox Family V2 gate as the loader.");
assert(lifePlanAttachmentRoute.includes("workspaceDisplayName: workspace.display_name") && lifePlanAttachmentRoute.includes("workspacePublicSlug: workspace.public_slug"), "Life Plan uploads should use the same Ryan/Fox Family V2 gate as the loader.");
assert(loader.includes(".select(\"id, slug, display_name, public_slug\")"), "Workspace resolution should expose public_slug to V2-gated API routes.");
...
assert(workspaceRoute.includes("id: workspaceAccess.workspace.id") && workspaceRoute.includes("slug: workspaceAccess.workspace.slug"), "DOS route should load data from the authorized resolved Ryan workspace.");
