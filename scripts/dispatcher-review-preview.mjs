...
export function getConfig(env = process.env, argv = process.argv.slice(2)) {
  ...
  return {
    ...
    enabled: parseBoolean(env.DISPATCHER_REVIEW_PREVIEW_ENABLED, true),
    dryRun: Boolean(args.dryRun) || parseBoolean(env.DISPATCHER_REVIEW_PREVIEW_TEST_MODE, false),
    singleWorkerMode: parseBoolean(env.DISPATCHER_SINGLE_WORKER_MODE, false),
...
async function pushReviewBranch(branch, config, options = {}) {
  if (!config.enabled) {
    return {
      ok: false,
      reason: "review preview publishing is disabled by DISPATCHER_REVIEW_PREVIEW_ENABLED=false",
    };
  }
