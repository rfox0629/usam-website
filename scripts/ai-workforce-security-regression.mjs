...
const dashboardContract = read("src/lib/operations-center/operations-center.ts");
...
assertIncludes(page, 'active="operations-center"', "Operations Center page must use the Operations Center admin nav key.");
assertIncludes(page, "loadOperationsCenterData", "Operations Center page must load server-side sanitized dashboard data.");
assertIncludes(page, "Linear issue source unavailable.", "Operations Center page must show unavailable data instead of samples.");

assertIncludes(adminShell, 'href: "/admin/operations-center"', "Operations Center must be available from admin navigation.");
...
assertIncludes(dashboardContract, 'import "server-only"', "Dashboard data loader must stay server-only.");
assertIncludes(dashboardContract, "LINEAR_API_KEY", "Dashboard loader must keep Linear access server-side.");
assertIncludes(dashboardContract, "safeErrorMessage", "Dashboard loader must sanitize external errors.");
assertNotIncludes(dashboardContract, "readFile", "Dashboard loader must not read dispatcher files directly.");
assertNotIncludes(dashboardContract, "child_process", "Dashboard loader must not execute shell commands.");
