...
function fieldClassName() {
  return "mt-2 min-h-12 w-full rounded-xl border border-stone-300 bg-white px-4 text-base text-stone-950 shadow-sm outline-none transition placeholder:text-stone-400 focus:border-usam-gold focus:ring-4 focus:ring-usam-gold/15 md:text-sm";
}

function textAreaClassName() {
  return "mt-2 min-h-[145px] w-full rounded-xl border border-stone-300 bg-white px-4 py-3 text-base leading-8 text-stone-950 shadow-sm outline-none transition placeholder:text-stone-400 focus:border-usam-gold focus:ring-4 focus:ring-usam-gold/15 md:text-sm md:leading-7";
}
...
function FieldLabel({
  ...
  return (
    ...
    >
      {children}
      {required ? <span className="ml-1 text-usam-gold">*</span> : null}
    </label>
  );
}
...
function ConsentCheckbox({
  ...
  return (
    <label className="flex gap-3 rounded-xl border border-stone-200 bg-white p-4 text-sm leading-7 text-stone-700 transition-colors hover:border-usam-gold/70">
      <input
        type="checkbox"
        name={name}
        required
...
export function FinancialFreedomInquiryForm({
  ...
  if (submitted) {
    return (
      <div className="rounded-[28px] border border-emerald-200 bg-emerald-50 p-7 text-base leading-8 text-emerald-950 shadow-sm md:p-10">
        ...
        {uploadPartial ? (
          <p className="mt-5 max-w-3xl rounded-xl border border-usam-gold/25 bg-usam-gold/20 p-4 text-sm leading-7 text-usam-gold">
  ...
  return (
    <form action={submitFinancialFreedomInquiry} className="rounded-[28px] border border-stone-200 bg-[#fbfaf7] p-4 shadow-[0_28px_90px_rgba(12,10,9,0.18)] md:p-6">
      ...
      >
        <div className="rounded-2xl border border-usam-gold/45 bg-[rgba(var(--usam-gold-rgb),0.16)] p-5 text-sm leading-7 text-stone-800 md:p-6">
      ...
      <SectionCard title="What Would You Like Help With?" description="Choose the areas where clarity would be most helpful right now.">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {helpAreas.map((area) => (
            <label key={area.name} className="flex min-h-12 items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm leading-6 text-stone-700 transition-colors hover:border-usam-gold/70">
              <input
                type="checkbox"
                name={area.name}
