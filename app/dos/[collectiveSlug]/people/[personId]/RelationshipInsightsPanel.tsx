  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/dos/${collectiveSlug}/people/${person.id}/insights`, {
        body: JSON.stringify({
          commitmentLevel,
          notesPrivate: person.notesPrivate ?? "",
          relationshipDepth,
        }),
        headers: {
        ...
        <section className="border border-stone-800 bg-[#080808] p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-stone-400">Where are they spiritually right now?</p>
            </div>
            ...
            {commitmentOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  type="button"
                >
                  <span className="text-[10px] font-bold uppercase tracking-[0.16em]" style={{ fontFamily: font.rajdhani }}>
...
export function RelationshipNotesPanel({
  collectiveSlug,
  person,
}: {
  collectiveSlug: string;
