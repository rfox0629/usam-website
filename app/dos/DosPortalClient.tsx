...
// USA-289: /dos is where signed-in people land. This chooser shows when an
// account can open more than one workspace, or none yet. Signed-out visitors
// never reach it: /dos sends them to the DOS sign-in page. The old
// "Start your DOS field" form, which only forwarded to sign-in and created
// nothing, is gone; requesting access is /dos/setup.
...
export function DosPortalClient({ launchWorkspaces = [] }: DosPortalClientProps) {
  return (
      ...
      <main className="mx-auto flex min-h-screen max-w-5xl flex-col justify-center gap-6 px-4 py-8 sm:px-6">
        <section className="max-w-2xl">
          ...
          <h1 className="mt-4 text-4xl font-bold leading-[0.95] tracking-[-0.045em] text-[#0F172A] sm:text-5xl">
            {launchWorkspaces.length ? "Choose your workspace" : "You're signed in"}
          </h1>
        ...
        {launchWorkspaces.length ? (
          ...
          <section className="rounded-[24px] border border-[#E2E8F0] bg-white p-5 shadow-[0_16px_42px_rgba(15,23,42,0.06)]">
            <h2 className="text-lg font-bold tracking-[-0.02em] text-[#0F172A]">Your DOS access isn&apos;t set up yet</h2>
            <p className="mt-2 text-sm leading-6 text-[#64748B]">
              This account isn&apos;t connected to a DOS workspace. If you&apos;ve requested access, it opens here once your request is approved.
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              <Link
                className="inline-flex min-h-11 items-center justify-center rounded-full bg-[#2563EB] px-5 text-sm font-semibold text-white transition hover:bg-[#1D4ED8]"
                href="/dos/setup"
              >
                Request DOS access
              </Link>
              <form action="/api/access/logout" method="post">
                <button className="inline-flex min-h-11 items-center justify-center rounded-full border border-[#E2E8F0] bg-white px-5 text-sm font-semibold text-[#0F172A]" type="submit">
                  Sign out
                </button>
              </form>
            </div>
