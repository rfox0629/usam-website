...
type SupportSubsection = "commitments" | "flyer-builder" | "giving-page" | "overview" | "settings" | "share-tools";
...
const supportSubsectionOptions: Array<{ label: string; value: SupportSubsection }> = [
  ...
  { label: "Flyer Builder", value: "flyer-builder" },
...
function hasRenderablePrayer(profile: AdminProfile) {
  return profile.show_prayer !== false;
}
...
function meetingDisplayStatusLabel(meeting: MeetingListItem) {
  if (meeting.status === "scheduled") {
    return "Upcoming";
  }

  if (meeting.nextStep && meeting.nextStep !== "Not set") {
  ...
  return meetingStatusLabel(meeting.status);
}

function meetingMatchesFilter(meeting: MeetingListItem, filter: MeetingsFilter) {
  if (filter === "all") {
    return true;
  }
  ...
  if (filter === "needs_follow_up") {
    return meeting.nextStep !== "Not set" && meeting.status !== "canceled";
  }

  return meeting.status === "completed" || meeting.status === "reviewed" || meeting.status === "fruit_created";
}

function meetingMatchesSearch(meeting: MeetingListItem, fieldPeople: readonly AdminFieldPerson[], searchValue: string) {
  ...
  return [
    ...
    meetingDisplayStatusLabel(meeting),
    meeting.nextStep,
    meeting.notes,
...
function MeetingDisplayStatusBadge({ meeting }: { meeting: MeetingListItem }) {
  const displayStatus = meetingDisplayStatusLabel(meeting);
...
function MeetingsManager({
  ...
  const [meetingsFilter, setMeetingsFilter] = useState<MeetingsFilter>("all");
  ...
  const filteredMeetings = useMemo(
    () => meetings.filter((meeting) => (
      meetingMatchesFilter(meeting, meetingsFilter)
        && meetingMatchesSearch(meeting, fieldPeople, meetingsSearch)
    )),
    [fieldPeople, meetings, meetingsFilter, meetingsSearch],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-xl border border-[#e2ded5] bg-white">
        ...
        <div className="flex flex-col gap-3 border-b border-[#e2ded5] bg-white px-4 py-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-wrap gap-2">
            {meetingsFilterOptions.map((option) => {
              const selected = meetingsFilter === option.value;
        ...
        {filteredMeetings.length > 0 ? (
          <div>
            <div className="hidden grid-cols-[118px_minmax(180px,1.4fr)_minmax(120px,0.85fr)_126px_126px_minmax(130px,1fr)_56px] items-center gap-3 border-b border-[#e2ded5] bg-[#fbfaf7] px-4 py-3 text-[9px] uppercase tracking-[0.16em] text-[#6f6658] xl:grid" style={{ fontFamily: font.rajdhani, fontWeight: 700 }}>
              <span>Date & Time</span>
              <span>Person</span>
            ...
            <div className="divide-y divide-[#e2ded5]">
              {filteredMeetings.map((meeting) => {
                ...
                return (
                  <div
                    className="grid min-h-[72px] cursor-pointer gap-3 bg-white px-4 py-4 transition-colors hover:bg-[#fbfaf7] xl:grid-cols-[118px_minmax(180px,1.4fr)_minmax(120px,0.85fr)_126px_126px_minmax(130px,1fr)_56px] xl:items-center"
                    key={meeting.id}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-3 xl:block">
                      ...
                      <div className="flex min-w-0 items-center justify-end gap-3 xl:justify-start">
                        ...
                        <div className="min-w-0 text-right xl:text-left">
                          ...
                          <span className="mt-1 block truncate text-xs leading-4 text-[#7b746a] xl:hidden">
                            {meetingTypeLabel(meeting.meetingType)}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="hidden min-w-0 xl:block">
                      <span className="block truncate text-sm leading-5 text-[#4b443b]">
                        {meetingTypeLabel(meeting.meetingType)}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 xl:block">
                      ...
                      <span className="block text-right text-sm leading-5 text-[#4b443b] xl:text-left">
                        {meetingDepthLabel(meeting.depth)}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 xl:block">
                      ...
                      <MeetingDisplayStatusBadge meeting={meeting} />
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 xl:block">
                      ...
                      <span className="block max-w-[220px] truncate text-right text-sm leading-5 text-[#4b443b] xl:max-w-full xl:text-left">{meeting.nextStep}</span>
                    </div>
      ...
      {selectedMeeting ? (
        <MeetingDetailModal
          ...
          tableEncounters={selectedTable ? encounters.filter((encounter) => encounter.table_id === selectedTable.id) : []}
          tableFruitItems={selectedTable ? fruitItems.filter((fruit) => fruit.table_id === selectedTable.id) : []}
          tableReview={selectedTable ? tableReviews.find((item) => item.table_id === selectedTable.id) ?? null : null}
        />
      ) : null}
...
function MeetingDetailModal({
  ...
  onUpdatePersonProfile: (person: AdminFieldPerson, patch: Partial<Pick<AdminFieldPerson, "church" | "engagement_level" | "relationship_type">>) => void;
  onUpdateReview: (tableId: string, patch: Partial<AdminTableReview>) => void;
  tableEncounters: readonly AdminEncounterSubmission[];
  tableFruitItems: readonly AdminFruitItem[];
  tableReview: AdminTableReview | null;
}) {
  const personLabel = meetingPeopleLabel(meeting, fieldPeople);
  const dateTime = meetingDateTimeParts(meeting);
  const [requestedSection, setRequestedSection] = useState<TableWorkflowSection | null>(null);
    ...
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm">
      <div className="ml-auto flex h-full w-full max-w-4xl flex-col border-l border-[#2a241a] bg-[#f8f6f1] text-[#111111] shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
        <div className="border-b border-[#e2ded5] bg-white px-4 py-4 md:px-6">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-2 flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-[#7b746a]">
                    {formatDosMeetingSecondary(meetingTypeLabel(meeting.meetingType), dateTimeDetailLabel(dateTime))}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button className={lightPrimaryButtonClass} onClick={editMeeting} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
              Edit Meeting
            </button>
            <button className={lightSecondaryButtonClass} onClick={onLogFollowUp} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
              Log Follow Up
            </button>
            {meeting.source === "table" ? (
              <>
                <button className={lightSecondaryButtonClass} onClick={() => setRequestedSection("review")} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
                  {reviewHasContent(tableReview) ? "View Review" : "Start Review"}
                </button>
                <button className={lightSecondaryButtonClass} onClick={() => setRequestedSection("fruit")} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
                  {tableFruitItems.length > 0 ? "View Fruit" : "Create Fruit"}
                </button>
              </>
            ) : null}
            <button className={lightSecondaryButtonClass} onClick={onClose} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-6">
          {meeting.table ? (
            <TableDetailPanel
              encounters={tableEncounters}
              fieldPeople={fieldPeople}
              fruitItems={tableFruitItems}
...
const meetingsFilterOptions: Array<{ label: string; value: MeetingsFilter }> = [
  { label: "All", value: "all" },
...
function TableDetailPanel({
  ...
  onUpdatePersonProfile: (person: AdminFieldPerson, patch: Partial<Pick<AdminFieldPerson, "church" | "engagement_level" | "relationship_type">>) => void;
  onUpdateReview: (tableId: string, patch: Partial<AdminTableReview>) => void;
  requestedSection?: TableWorkflowSection | null;
  review: AdminTableReview | null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <SupportMetricCard label="Percent Funded" value={`${progressPercentage}%`} />
        <SupportMetricCard label="Giving Page Status" value={givingPageStatus} />
      </div>
      ...
      <div className="rounded-2xl border border-[#e2ded5] bg-white p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <a className={`${lightPrimaryButtonClass} min-h-10 gap-2`} href={supportLink} rel="noopener noreferrer" style={{ fontFamily: font.rajdhani, fontWeight: 700 }} target="_blank">
            <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
            Open Giving Page
          </a>
          <button className={`${lightSecondaryButtonClass} min-h-10 gap-2`} onClick={() => onCopy(supportLink, "Support Link")} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
          ...
          <button className={`${lightTertiaryButtonClass} min-h-10`} onClick={() => onCopy(flyerLink, "Flyer Link")} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
  ...
  return (
    <article className="rounded-2xl border border-[#dcd6ca] bg-white p-4 shadow-[0_8px_22px_rgba(17,17,17,0.035)]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <h4 className="text-base font-semibold leading-tight text-[#111111]">{tool.title}</h4>
          <p className="mt-1 text-sm leading-5 text-[#6f6658]">{tool.description}</p>
        </div>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          className={`${lightSecondaryButtonClass} min-h-9 gap-2`}
          onClick={() => onCopy(tool.body, tool.title)}
          style={{ fontFamily: font.rajdhani, fontWeight: 700 }}
          type="button"
        >
          <Copy className="h-3.5 w-3.5" aria-hidden="true" />
        ...
        {tool.href ? (
          <a className={`${lightTertiaryButtonClass} min-h-9`} href={tool.href} rel="noopener noreferrer" style={{ fontFamily: font.rajdhani, fontWeight: 700 }} target="_blank">
            <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
            Preview
          </a>
        ) : onPreview ? (
          <button className={`${lightTertiaryButtonClass} min-h-9`} onClick={onPreview} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
...
function SupportShareTools({
  ...
  const [selectedTool, setSelectedTool] = useState<ShareTool | null>(null);
  const tools: ShareTool[] = [
    {
      body: textTemplate,
      description: "Short personal invite for quick follow-up.",
      icon: Smartphone,
      title: "Text Message",
    },
    {
      body: emailTemplate,
      description: "Longer support invitation with mission context.",
      icon: Mail,
      title: "Email",
    },
    {
      body: socialTemplate,
      description: "Ready for Facebook, Instagram, or a group post.",
      icon: MessageCircle,
      title: "Social Caption",
    },
    {
      body: flyerPrayerAsk,
      description: "Simple ask for prayer and monthly partnership.",
      icon: BookOpen,
      title: "Prayer Ask",
    },
    {
      body: supportLink,
      description: "Direct giving section link.",
      href: supportLink,
      icon: Heart,
      title: "Support Link",
    },
    {
      body: flyerLink,
      description: "Shareable public support flyer.",
      href: `${flyerLink}?version=color`,
      icon: FileText,
      title: "Flyer",
    },
  ];
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {tools.map((tool) => (
          <ShareToolCard
            key={tool.title}
            onCopy={onCopy}
            onPreview={tool.href ? undefined : () => setSelectedTool(tool)}
            tool={tool}
          />
        ))}
      </div>

      <div className="rounded-2xl border border-[#dcd6ca] bg-[#fdfbf7] p-3.5">
        <div className="grid gap-2 md:grid-cols-3">
          {[
            { label: "Profile Link", value: profileLink },
            { label: "Annual Goal", value: formatCurrency(annualGoal) },
            { label: "Support Link", value: supportLink },
          ].map((item) => (
            <div className="min-w-0 rounded-xl border border-[#e2ded5] bg-white p-3" key={item.label}>
              <p className="text-[10px] uppercase tracking-[0.18em] text-[#8a8174]" style={{ fontFamily: font.rajdhani, fontWeight: 700 }}>{item.label}</p>
              <p className="mt-1 truncate text-sm text-[#3f3932]">{item.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {selectedTool ? (
        <DetailModalShell eyebrow="Preview/Edit" onClose={() => setSelectedTool(null)} title={selectedTool.title}>
          ...
          <div className="mt-3 flex justify-end">
            <button
              className={`${lightPrimaryButtonClass} min-h-10 gap-2`}
              onClick={() => onCopy(selectedTool.body, selectedTool.title)}
              style={{ fontFamily: font.rajdhani, fontWeight: 700 }}
              type="button"
            >
              <Copy className="h-3.5 w-3.5" aria-hidden="true" />
...
function SupportFlyerBuilder({
  ...
  const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=260x260&margin=10&data=${encodeURIComponent(supportLink)}`;
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(360px,430px)] lg:items-start">
        ...
        <div className="rounded-2xl border border-[#dcd6ca] bg-white p-4">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            <button className={`${lightSecondaryButtonClass} min-h-10 gap-2`} onClick={() => onCopy(flyerLink, "Flyer Link")} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
              <LinkIcon className="h-3.5 w-3.5" aria-hidden="true" />
              Copy Flyer Link
            </button>
            ...
            <button className={`${lightTertiaryButtonClass} min-h-10`} onClick={() => onCopy(emailTemplate, "Email Template")} style={{ fontFamily: font.rajdhani, fontWeight: 700 }} type="button">
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-[#e2ded5] bg-white">
        {prayerRequests.length === 0 ? (
          <p className="p-4 text-sm leading-6 text-[#7b746a]">No prayer requests yet.</p>
        ) : (
          prayerRequests.map((request) => (
            <div className="border-b border-[#e2ded5] p-3.5 last:border-b-0" key={request.id}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <p className="text-base font-semibold text-[#111111]">{request.title}</p>
                  <p className="mt-1 line-clamp-2 text-sm leading-6 text-[#4b443b]">{request.request}</p>
