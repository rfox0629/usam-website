...
function TeachingResource({ resource }: GuideResourceClientProps) {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#F8FBFF_0%,#FFFFFF_46%,#FFF7ED_100%)] px-4 py-5 text-[#0F172A]">
      <article className="mx-auto max-w-xl pb-8">
        <header className="rounded-[34px] border border-white/80 bg-white/95 p-5 shadow-[0_24px_70px_rgba(37,99,235,0.10)] backdrop-blur">
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-[#EBF2FF] px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-[#2563EB]">
              {resource.category}
            </span>
            <span className="rounded-full border border-[#DCEBFF] bg-white px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-[#475569]">
              Teaching
            </span>
          </div>
          <h1 className="mt-4 text-[42px] font-black leading-[0.92] tracking-[-0.045em] text-[#0F172A] sm:text-5xl">
            {resource.title}
          </h1>
          ...
          {resource.content?.scripture ? (
            <p className="mt-4 rounded-2xl border border-[#DCEBFF] bg-[#F8FBFF] px-3 py-2 text-sm font-black text-[#1D4ED8]">
        ...
        <div className="mt-4 grid gap-3">
          {sections.map((section, sectionIndex) => (
            <section className="rounded-[28px] border border-[#EAF2FF] bg-white p-4 shadow-[0_16px_44px_rgba(37,99,235,0.055)]" key={section.title}>
              <div className="flex items-start gap-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#EBF2FF] text-sm font-black text-[#2563EB]">
                  {sectionIndex + 1}
                </span>
                <div className="min-w-0 flex-1">
                  <h2 className="text-lg font-black leading-6 text-[#0F172A]">{section.title}</h2>
              ...
              {section.items?.length ? (
                <div className="mt-4 grid gap-2">
                  {section.items.map((item) => (
                    <div className="rounded-[22px] border border-[#DCEBFF] bg-[#F8FBFF] p-3" key={item.title}>
          ...
          {resource.content?.attribution ? (
            <footer className="rounded-[24px] border border-[#DCEBFF] bg-white p-4 text-sm leading-6 text-[#475569] shadow-[0_12px_34px_rgba(37,99,235,0.045)]">
              <p className="font-bold text-[#0F172A]">{resource.content.attribution}</p>
              {resource.content.credits ? <p className="mt-1 text-xs font-semibold text-[#64748B]">{resource.content.credits}</p> : null}
            </footer>
          ) : null}
        </div>
      </article>
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2">
      {references.map((reference) => (
        <span className="rounded-full bg-[#EBF2FF] px-3 py-1.5 text-xs font-bold text-[#1D4ED8]" key={reference}>
          {reference}
        </span>
      ))}
    </div>
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#F8FBFF_0%,#FFFFFF_46%,#FFF7ED_100%)] px-4 py-5 text-[#0F172A]">
      <article className="mx-auto flex min-h-[calc(100vh-2.5rem)] max-w-xl flex-col justify-center pb-8">
        <section className="rounded-[34px] border border-white/80 bg-white/95 p-5 shadow-[0_24px_70px_rgba(37,99,235,0.10)] backdrop-blur">
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-[#EBF2FF] px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.16em] text-[#2563EB]">
              {resource.category}
            </span>
          ...
          {!started ? (
            <div className="mt-5 rounded-[26px] border border-[#DCEBFF] bg-[#F8FBFF] p-4">
              <p className="text-sm font-black text-[#0F172A]">0-10 scale</p>
              <p className="mt-2 text-sm leading-6 text-[#475569]">
                {resource.content?.assessmentScale ?? "Each person answers the same questions and receives a score out of 150."}
              </p>
              <button
                className="mt-4 inline-flex min-h-12 w-full items-center justify-center rounded-full bg-[linear-gradient(135deg,#2563EB_0%,#1D4ED8_100%)] px-5 text-sm font-black text-white shadow-[0_14px_28px_rgba(37,99,235,0.22)]"
          ...
          {started && !isComplete && question ? (
            <div className="mt-5">
              <div className="rounded-full bg-[#EAF2FF] p-1">
                <div className="h-2 rounded-full bg-[#2563EB] transition-all" style={{ width: `${progressPercent}%` }} />
              </div>
              ...
              <h2 className="mt-3 text-2xl font-black leading-8 text-[#0F172A]">
                {questionPrompt(question, participant)}
              </h2>
              {question.note ? <p className="mt-3 rounded-2xl border border-[#FED7AA] bg-[#FFF7ED] px-3 py-2 text-xs font-bold leading-5 text-[#9A3412]">{question.note}</p> : null}
                    ...
                    <button
                      ...
                      className={`h-11 rounded-2xl text-sm font-black transition-colors ${
                        selected
                          ...
                          : "border border-[#DCEBFF] bg-white text-[#1D4ED8] hover:bg-[#EBF2FF]"
                      }`}
                      key={score}
              ...
              <div className="mt-5 grid grid-cols-[0.8fr_1fr] gap-2">
                <button
                  className="inline-flex min-h-12 items-center justify-center rounded-full border border-[#DCEBFF] bg-white px-4 text-sm font-black text-[#0F172A]"
                ...
                <button
                  className="inline-flex min-h-12 items-center justify-center rounded-full bg-[linear-gradient(135deg,#2563EB_0%,#1D4ED8_100%)] px-4 text-sm font-black text-white shadow-[0_14px_28px_rgba(37,99,235,0.22)] disabled:cursor-not-allowed disabled:opacity-50"
          ...
          {isComplete ? (
            <div className="mt-5 space-y-3">
              {results.map((result) => (
                <section className="rounded-[26px] border border-[#DCEBFF] bg-[#F8FBFF] p-4" key={result.name}>
                  <div className="flex items-baseline justify-between gap-3">
                    <h2 className="text-lg font-black text-[#0F172A]">{result.name}</h2>
                    <p className="text-2xl font-black text-[#2563EB]">{result.total}/{assessment.maxScore}</p>
                  </div>
                  <p className="mt-3 text-[11px] font-black uppercase tracking-[0.16em] text-[#64748B]">Growth areas</p>
                  <div className="mt-2 grid gap-2">
                    {result.growthAreas.map(({ question: item, score }) => (
                      <div className="rounded-2xl bg-white px-3 py-2 text-sm leading-5 text-[#475569]" key={item.id}>
                        <span className="font-black text-[#0F172A]">{score}/10</span> - {questionPrompt(item, result.name)}
                      </div>
                    ))}
                  </div>
                </section>
              ...
              <div className="grid gap-2">
                <button
                  className="inline-flex min-h-12 items-center justify-center rounded-full bg-[linear-gradient(135deg,#2563EB_0%,#1D4ED8_100%)] px-4 text-sm font-black text-white shadow-[0_14px_28px_rgba(37,99,235,0.22)]"
                  onClick={copyResults}
                  type="button"
                >
                  Copy Results
