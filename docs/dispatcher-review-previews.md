...
Use `VERCEL_TEAM_ID` when the Vercel project belongs to a team.
...
`In Review` may remain the Linear state for review bookkeeping, but it is not a founder bottleneck for unrelated eligible dispatcher work once code validation, protected preview publication, and engineering approval are complete.
...
2. Local branch exists and has a diff from `origin/main`.
