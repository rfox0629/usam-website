...
Optional pieces that make the page read as a product rather than a report:
...
no new image to the repository. A named real person in `spotlight` or
`network` appears there by their own choice, and the `disclaimer` must say so
and must say that every other person in the tree is a placeholder.
