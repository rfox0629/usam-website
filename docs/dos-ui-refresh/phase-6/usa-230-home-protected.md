...
- `mobile--home.png` was recorded once (USA-215, commit `add200b`) and stayed byte-identical through every later PR's `npm run test:dos:visual` (USA-216, 217, 218, 222, 220, 223, 227, 226, 229, 225, and the Phase 8 fixes). It was re-recorded once, in the Phase 8 release re-verification on 2026-09-05, because the local calendar day rolled over during the run: the only differing pixels were the date string ("Friday, September 4" → "Saturday, September 5"), confirmed by a pixel diff before re-recording (P-9 drift, not a Home change).
