...
| S-1a | `dos.disabled` moved from V10's `#9AA3B2` to `#68707F` (USA-280). At `#9AA3B2` a disabled control's own label sat at 2.5:1 on `dos.surface2`; `#68707F` reaches 4.6:1 and is still visibly lighter than `dos.secondary`, so a disabled state still reads as disabled. | USA-280: "preserve distinguishable disabled states without making text unreadable." Issue outranks the spec and V10 (root `CLAUDE.md`). | Yes — one value in two files. |
...
| C-3 | Ryan (2026-09-11, USA-271): a group gathering is recorded from the gathering card itself. The primary action on a group is **Take Attendance**; opening a scheduled occurrence creates nothing and marks nobody present (a person counts only when the leader ticks them); the sheet holds the date and start time, a compact duration prefilled from the schedule, the attendance checklist from the roster, optional "What we covered" with a Journey link, notes, and repeatable prayer requests; one **Save gathering** appears only when something changed; unscheduled gatherings and guests stay discreet. The record is the canonical `dos_group_gatherings` row (status completed) plus one `dos_group_attendance` row per person (unique per gathering, so a retry never duplicates; a person unmarked later is kept as absent) plus canonical `prayer_requests` rows linked by gathering; the leader's separate meeting log is untouched. A Person timeline shows "Attended <group> · <date>" from attendance and opens the gathering; Reports counts distinct gatherings attended ("Attended N group gatherings"), never multiplies leader time, and attendance is not Ministry Fruit. A Journey link never completes anyone's own assignment. "What we covered" and the Journey link need three nullable columns (migration `20260911160000_dos_group_gathering_covered_journey`), which is committed but not applied: the loader probes for the columns and the sheet hides both fields until the migration is authorised and applied. "Just the two of you" reads "No ministry team recorded". | `GroupGatheringSheet`, `app/api/dos/app/groups/gatherings/route.ts` (`action: "record"`), `ministry-report.ts` (`gatherings`), `scripts/dos-groups-regression.mjs`, `docs/dos-ui-refresh/usa-271/` |
