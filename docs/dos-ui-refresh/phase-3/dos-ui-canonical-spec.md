...
- Date: compact DateInput ("Today, Sep 4"). Who was there: Chips from the person picker, household spouse present but unselected, "+ Add". Duration: Stepper, 15-min steps, default = production's 30m *(unresolved: default)*. How did you connect: Select, default = production "In person" *(V10 shows Coffee; keep production)*. Prayer: ToggleRow "We prayed together" + ToggleRow "Add something to pray for" revealing a Field (errors if ticked and empty). Fruit observed (optional Select) *(unresolved: on form vs afterward — keep where production has it)*. What happened: VoiceTextarea. For USAM workspaces only (decided from actual application / profile / owning-organization state in `src/lib/dos/usam-workspace.ts` and enforced by the meetings API with a 403, not by client visibility alone — USA-238), one collapsed optional **Kitchen Table Gospel Responses** row (title, the description "Questions, spiritual gifts, and ministry outcomes", and a status line reading "Not added" or a concise summary) opens the Kitchen Table questions in conversational order (the three collapsed gift groups sit directly under "Do you have any spiritual gifts?" and render only while it is Yes), the relationship rating, and one optional collapsed "Add significant outcomes" row with a flat nine-item list. It never exposes a flow chooser, a discussion-guide picker, or Four Questions. Outcomes record what happened in that conversation and live with the meeting; Observed Fruit remains the leader's separate assessment, and outcomes never create, update or duplicate a Fruit record. Meeting context, next steps, and review link options retain their existing behavior (B8). *[v1.2, USA-242]* Accountability from a meeting is one compact `+ Accountability` row; its editor leads with "What are they working toward?" (no taxonomy required), a collapsed "Need an idea?" disclosure (focus chips → suggestions), one Tracking select (Check in regularly / Reach a target / Complete once) revealing only its fields; exactly one editor is open at a time, saved drafts collapse to a summary row (title, "Monthly · Starts Oct 7") with an overflow menu holding Edit and Remove, and a compact "+ Add another" beneath; drafts persist until the meeting is logged and submit through the unchanged USA-235 field contract. The same editor is the Person Add/Edit Accountability sheet.
