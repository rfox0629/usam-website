...
| --- | --- | --- |
| B1 | **Home (mobile) and Dashboard (desktop) are unchanged** in layout, content, and order for the life of this project. Only shared tokens may touch them, and only when a re-screenshot shows no visible change beyond color/typography values. *[Superseded for the approved Home sections by USA-257, settled by Ryan on 2026-09-09 — see decision log §F; the circle hero and navigation are still protected.]* | `CircleFocusHero`, `DesktopHomeDashboard` |
...
| B7 | **Editable surfaces never lose entered work.** A surface declares `kind="editable"` or is a task screen; its backdrop and swipe are inert; every deliberate exit is routed through `useUnsavedWorkGuard`; the discard dialog appears only when the form is dirty (real entered work, not any keystroke); Keep editing is the safe default. Inspection surfaces close on backdrop. *[v1.3, USA-269: "dirty" is measured from the user's first interaction with the surface, so a form still initialising is never dirty; search, filter and other viewing controls never count; a surface that stays open after a successful save re-baselines; the dialog reads "Leave without saving?" — "Your unsaved changes will be lost. Anything already saved will stay." — Keep editing / Leave without saving.]* | `src/lib/dos/unsaved-work.ts`, `Sheet`, `DosWorkflowPage` |
...
| **PillRail** | Scrollable pill tab rail: 36px pills, 15px padding, 6px gap, 13.5/600 ink on white with hairline; active = blue fill, white text; native horizontal scroll, no arrows, right-edge fade **only while the rail overflows** *[v1.1: a short rail painted a white strip over tinted grounds]*, selected pill scrolls into view, offset retained on return; 44px hit area; `role=tablist`; `edgeInset` 4 or 5 matches the container padding. Used by My Record, Field circles, Prayer tabs, and Meetings Calendar/Timeline. | `src/components/dos/ui/PillRail.tsx` (adopted by Meetings and My Record in Phase 5); legacy `PeopleCircleTabs`, `SegmentedTabs` until Phase 6 |
| **Segmented** | For 2–3 exclusive views inside content (month/week and Person Overview/Timeline/Details): `surface-2` track, 4px padding, `r3`; fixed Person views are centered and evenly divided. | `SegmentedTabs` |
...
| **Stepper** | − value + ; blue-50 end regions with hairline dividers and a white centered value; fixed steps (15 min for durations, 5 min for Time with God); no ceiling; confirm past a threshold (threshold is unresolved, §9). *[v1.2]* 56px tall in three equal regions with the value and both buttons vertically centered (USA-238 review). | new (replaces duration pills) |
...
- Date: compact DateInput ("Today, Sep 4"). Who was there: Chips from the person picker, household spouse present but unselected, "+ Add". Duration: Stepper, 15-min steps, default = production's 30m *(unresolved: default)*. How did you connect: Select, default = production "In person" *(V10 shows Coffee; keep production)*. Prayer: ToggleRow "We prayed together" + ToggleRow "Add something to pray for" revealing a Field (errors if ticked and empty). Fruit observed (optional Select) *(unresolved: on form vs afterward — keep where production has it)*. What happened: VoiceTextarea. For USAM workspaces only (decided from actual application / profile / owning-organization state in `src/lib/dos/usam-workspace.ts` and enforced by the meetings API with a 403, not by client visibility alone — USA-238), one collapsed optional **Kitchen Table Gospel Responses** row (title, the description "Questions, spiritual gifts, and relationship rating", and a status line reading "Not added" or a concise summary) opens the Kitchen Table questions in conversational order (the three collapsed gift groups sit directly under "Do you have any spiritual gifts?" and render only while it is Yes) and the relationship rating. It never exposes a flow chooser, a discussion-guide picker, Four Questions, or an outcome picker *[v1.2, USA-243: Kitchen Table Gospel has no outcome capture of its own; the meeting's Observed Fruit disclosure is the only leader-facing fruit entry point; historical Kitchen Table outcomes still render in meeting detail]*. Meeting context, next steps, and review link options retain their existing behavior (B8). *[v1.2, USA-242]* Accountability from a meeting is one compact `+ Accountability` row; its editor leads with "What are they working toward?" (no taxonomy required), a collapsed "Need an idea?" disclosure (focus chips → suggestions), one Tracking select (Check in regularly / Reach a target / Complete once) revealing only its fields; exactly one editor is open at a time, saved drafts collapse to a summary row (title, "Monthly · Starts Oct 7") with an overflow menu holding Edit and Remove, and a compact "+ Add another" beneath; drafts persist until the meeting is logged and submit through the unchanged USA-235 field contract. The same editor is the Person Add/Edit Accountability sheet.
