...
7. Issue moves to In Review for engineering review bookkeeping after a protected preview URL exists.
8. Eligible unrelated dispatcher work continues after engineering approval. Founder approval is required only for strategic/product-experience decisions, merge, production deploy, DNS, production schema mutation, or controls.
...
- V1 route and status endpoint are read-only and may publish protected previews after local validation passes.
...
- Merge to main, production deploy, DNS, production database/schema mutation, and external communications remain outside this packet.
- Linear In Review is not a founder bottleneck for unrelated eligible work after protected preview publication and engineering approval.
