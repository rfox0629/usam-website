...
`Build -> Validate -> Protected Preview URL -> Engineering Review -> Founder Experience Review -> Merge/Production`
...
Current blocker from the USA-60 lane: `dispatcher/usa-52-codex-20260721154907` exists only as a local dispatcher worktree branch. GitHub branch search returned no remote `usa-52` branch, and this lane is forbidden from pushing or deploying. Required action: push the USA-52 dispatcher branch to `origin` or rerun the dispatcher with push enabled so Vercel can create the protected preview deployment.
