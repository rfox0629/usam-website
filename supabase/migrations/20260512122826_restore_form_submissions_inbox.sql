...
create table if not exists public.form_submissions (
  ...
  assigned_team text not null default 'support_team',
  source_page text,
  name text,
  first_name text,
  last_name text,
...
alter table public.form_submissions
  ...
  add column if not exists assigned_team text default 'support_team',
...
set
  form_type = coalesce(nullif(form_type, ''), 'general'),
...
alter table public.form_submissions
  ...
  check (status in ('new', 'reviewed', 'needs_follow_up', 'follow_up', 'contacted', 'converted', 'closed', 'archived'));
...
comment on table public.form_submissions is
  'Shared NCC inbox mirror for public forms. Canonical records remain in their domain tables such as support_commitments and major_gift_inquiries.';;
